...
int main(int argc, char **argv){
	...
	double back_dist = 1.;
	double dist_unclustered = 2.;
	double cut_ucp_ratio = 0.10;
	...
	for (int eang = 0; eang < EANG.size(); ++eang){
				...
				for (int c = 0; c < num_clusters; ++c){
					...
					while(pl_size != 0){
						...
						if (ord_trk.size() <= ang_points){
							for (int j = 0; j < pl_size; ++j){
								...
								weight = pow(euler, -dist/EDIST[edist]) * pow(euler, -cone_dotP/EANG[eang]);
					...
					for (unsigned i = 0; i < points_gd.size(); ++i) {
						...
						for (unsigned j = 0; j < ord_trk.size(); ++j) {
							...
							if(dist < dist_unclustered) {
								close_ucp_found = true;
								break;
							}
						}
						if(close_ucp_found) num_close_unclustered += 1.;
					}

					double far_ucp = ord_trk.size() + points_gd.size() - num_close_unclustered;
					double far_ucp_ratio = far_ucp / (ord_trk.size() + points_gd.size());
		...
		/*
			...
			cout << "Purity = " << purity << ", Efficiency = " << efficiency << ", Vertex RMS = " << ver_rms << ", Vertex Mean = " << ver_mean << endl;
				cout << "Epsilon phi = "<< EANG[eang] <<"; Epsilon dist = " << EDIST[edist] << endl;
			cout << "####################################################################" << endl;
		*/
